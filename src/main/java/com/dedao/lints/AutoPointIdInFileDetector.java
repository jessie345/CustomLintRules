...
public class AutoPointIdInFileDetector extends LayoutDetector {
     ...
     */
    public static final Issue DEDAO_WITHIN_LAYOUT = Issue.create(
            "DuplicateIds",
     ...
     * The main issue discovered by this detector
     */
    public static final Issue DEDAO_CROSS_LAYOUT = Issue.create(
    ...
    public void afterCheckProject(@NonNull Context context) {
        if (context.getPhase() == 1) {
            ...
            if (!mIncludes.isEmpty()) {
                ...
                // across.
                if (context.isEnabled(DEDAO_CROSS_LAYOUT)
                ...
                for (Occurrence occurrence : mErrors) {
                    ...
                    if (location == null) {
                        ...
                        if (clientData instanceof Node) {
                            ...
                            if (context.getDriver().isSuppressed(null, DEDAO_CROSS_LAYOUT, node)) {
                                continue;
                            }
                        }
                    }
                    ...
                    context.report(DEDAO_CROSS_LAYOUT, location, occurrence.message);
                }
            }
        }
    }
...
//        }
        ...
        if (context.getPhase() == 1) {
            if (mIds.contains(id)) {
                ...
                context.report(DEDAO_WITHIN_LAYOUT, attribute, location,
            ...
            if (maps != null && !maps.isEmpty()) {
                for (Multimap<String, Occurrence> map : maps) {
                    if (!maps.isEmpty()) {
                        ...
                        if (occurrences != null && !occurrences.isEmpty()) {
                            for (Occurrence occurrence : occurrences) {
                                if (context.getDriver().isSuppressed(context, DEDAO_CROSS_LAYOUT,
                                        attribute)) {
                                    return;
                                }
                                Location location = context.getLocation(attribute);
